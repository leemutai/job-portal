        ...
        <section className="w-full lg:w-3/4 text-gray-800 px-4 lg:px-0">
          ...
          {jobs.length > 0 && (
            ...
            {Array.from({length: Math.ceil(jobs.length / 6)}).map((_ , index)=>(
                <a href="#job-list">
                    <button onClick={()=> setCurrentPage(index + 1)} className={`w-10 h-10 flex items-center justify-center border border-gray-300 rounded ${currentPage === index + 1 ? 'bg-blue-100 sm:text-blue-500' : 'text-gray-500'} `}>{index + 1}</button>
                </a>
            ))}
            <a href="#job-list">
